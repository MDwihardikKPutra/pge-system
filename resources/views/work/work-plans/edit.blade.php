...
@section('page-title', 'Edit Rencana Kerja')
@section('page-subtitle', 'Ubah detail rencana kerja')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3 mb-4">
        ...
        <h1 class="text-2xl font-bold text-slate-800">Edit Rencana Kerja</h1>
    </div>

    <form action="{{ route('user.work-plans.update', $workPlan) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white shadow-sm rounded-lg border border-slate-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" id="plan_date" name="plan_date" value="{{ old('plan_date', $workPlan->plan_date->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <select id="work_location" name="work_location" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <option value="office" {{ old('work_location', $workPlan->work_location?->value) == 'office' ? 'selected' : '' }}>Office (Kantor)</option>
                        <option value="site" {{ old('work_location', $workPlan->work_location?->value) == 'site' ? 'selected' : '' }}>Site (Lapangan)</option>
                        <option value="wfh" {{ old('work_location', $workPlan->work_location?->value) == 'wfh' ? 'selected' : '' }}>WFH (Work From Home)</option>
                        <option value="wfa" {{ old('work_location', $workPlan->work_location?->value) == 'wfa' ? 'selected' : '' }}>WFA (Work From Anywhere)</option>
                    </select>
                    @error('work_location')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-4">
                ...
                <input type="number" id="planned_duration_hours" name="planned_duration_hours" value="{{ old('planned_duration_hours', $workPlan->planned_duration_hours) }}" 
        ...
        <div class="bg-white shadow-sm rounded-lg border border-slate-200 p-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" required
                    ...
                    class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">{{ old('description', $workPlan->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-4">
                ...
                <textarea id="expected_output" name="expected_output" rows="3"
                    ...
                    class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">{{ old('expected_output', $workPlan->expected_output) }}</textarea>
            </div>
        </div>
