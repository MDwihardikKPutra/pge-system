...
async function loadLeaveDetailData(id) {
    ...
    const url = `/admin/approvals/leaves/${id}`;
        ...
        if (result.success && result.data) {
            ...
            if (footer) {
                if (data.status === 'pending') {
                    footer.innerHTML = generateLeaveApprovalFooter(id);
                } else {
                    footer.innerHTML = '<button type="button" onclick="closeLeaveDetailModal()" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Tutup</button>';
                }
            }
        } else {
            if (content) {
            ...
            <div class="border rounded-lg bg-gray-50 p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <div class="text-gray-700">${data.remaining_leave} hari</div>
                    </div>
                </div>
                ...
                <div class="mt-4 pt-4 border-t">
                    ...
                    <div class="flex items-center justify-between bg-white p-3 rounded border">
                        ...
                        <a href="${data.attachment_url}" target="_blank" class="text-xs font-medium hover:underline" style="color: #0a1628;">Download</a>
                    </div>
                </div>
                ` : ''}
            </div>
...
function generateLeaveApprovalFooter(id) {
    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.content || '';
    
    return `
        <div class="flex justify-between items-center w-full">
            <div class="flex-1 mr-4">
                <form id="approve-leave-form-${id}" method="POST" action="/admin/approvals/leaves/${id}/approve" class="flex gap-2" onsubmit="event.preventDefault(); submitApproveLeaveForm(${id});">
            ...
            <button type="button" onclick="closeLeaveDetailModal()" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 ml-2">
                Tutup
            </button>
        </div>
    `;
...
async function submitApproveLeaveForm(id) {
    ...
    const actionUrl = `/admin/approvals/leaves/${id}/approve`;
...
function submitLeaveRejectForm(event) {
    ...
    const actionUrl = `/admin/approvals/leaves/${id}/reject`;
