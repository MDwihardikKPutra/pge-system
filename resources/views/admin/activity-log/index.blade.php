        ...
        <div class="px-6 py-4 border-b border-slate-200" style="background-color: #0a1628;">
            ...
            <form method="GET" action="{{ route('admin.activity-log.index') }}" class="flex flex-wrap items-center gap-2">
                <div class="relative">
                    ...
                    <select name="user_id" class="px-2 py-1.5 text-xs border border-gray-600 bg-slate-800 text-white rounded focus:ring-1 focus:ring-blue-400 focus:border-blue-400" style="min-width: 120px;">
                    ...
                    @if(request()->hasAny(['search', 'user_id', 'action', 'model_type', 'date_from', 'date_to']))
                        <a href="{{ route('admin.activity-log.index') }}" class="px-3 py-1.5 text-xs font-medium text-gray-300 bg-slate-700 hover:bg-slate-600 rounded transition-colors">Reset</a>
                    @endif
                </form>
        </div>
                        ...
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <span class="text-xs font-medium text-slate-900">{{ $log->user->name ?? 'System' }}</span>
                                    <span class="text-[10px] text-slate-500">({{ $log->user->email ?? 'N/A' }})</span>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-xs text-slate-900">{{ $log->ip_address ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
