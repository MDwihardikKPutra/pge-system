...
<div class="py-8">
    ...
    <div class="mb-6 flex gap-2 border-b border-gray-200">
        <a href="{{ route('user.payment-approvals.index', ['type' => 'all', 'status' => request('status', 'pending')]) }}" 
        ...
        <a href="{{ route('user.payment-approvals.index', ['type' => 'spd', 'status' => request('status', 'pending')]) }}" 
        ...
        <a href="{{ route('user.payment-approvals.index', ['type' => 'purchase', 'status' => request('status', 'pending')]) }}" 
        ...
        <a href="{{ route('user.payment-approvals.index', ['type' => 'vendor-payment', 'status' => request('status', 'pending')]) }}" 
    ...
    <div class="mb-4 flex gap-2">
        <a href="{{ route('user.payment-approvals.index', ['type' => request('type', 'all'), 'status' => 'all']) }}" 
        ...
        <a href="{{ route('user.payment-approvals.index', ['type' => request('type', 'all'), 'status' => 'pending']) }}" 
        ...
        <a href="{{ route('user.payment-approvals.index', ['type' => request('type', 'all'), 'status' => 'approved']) }}" 
        ...
        <a href="{{ route('user.payment-approvals.index', ['type' => request('type', 'all'), 'status' => 'rejected']) }}" 
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3 text-center">
                            ...
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('user.payment-approvals.spd.approve', $spd) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="text-xs text-green-600 hover:text-green-800 font-medium">Setujui</button>
                                </form>
                                <span class="text-gray-300">|</span>
                                <button onclick="openRejectModal('spd', {{ $spd->id }})" class="text-xs text-red-600 hover:text-red-800 font-medium">Tolak</button>
                            </div>
                            @else
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3 text-center">
                            @if($purchase->isPending())
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('user.payment-approvals.purchase.approve', $purchase) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="text-xs text-green-600 hover:text-green-800 font-medium">Setujui</button>
                                </form>
                                <span class="text-gray-300">|</span>
                                <button onclick="openRejectModal('purchase', {{ $purchase->id }})" class="text-xs text-red-600 hover:text-red-800 font-medium">Tolak</button>
                            </div>
                            @else
                            <span class="text-xs text-gray-400">-</span>
                            @endif
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3 text-center">
                            @if($vp->isPending())
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('user.payment-approvals.vendor-payment.approve', $vp) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="text-xs text-green-600 hover:text-green-800 font-medium">Setujui</button>
                                </form>
                                <span class="text-gray-300">|</span>
                                <button onclick="openRejectModal('vendor-payment', {{ $vp->id }})" class="text-xs text-red-600 hover:text-red-800 font-medium">Tolak</button>
                            </div>
                            @else
                            <span class="text-xs text-gray-400">-</span>
                            @endif
...
<!-- Reject Modal -->
@include('user.payment-approvals.reject-modal')
...
function submitRejectForm(event) {
    ...
    const baseUrl = '{{ url("/") }}';
    if (type === 'spd') {
        actionUrl = baseUrl + '/{{ $routePrefix }}/payment-approvals/spd/' + id + '/reject';
    } else if (type === 'purchase') {
        actionUrl = baseUrl + '/{{ $routePrefix }}/payment-approvals/purchases/' + id + '/reject';
    } else if (type === 'vendor-payment') {
        actionUrl = baseUrl + '/{{ $routePrefix }}/payment-approvals/vendor-payments/' + id + '/reject';
    }
