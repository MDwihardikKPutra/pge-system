...
@section('title', 'Daftar Cuti & Izin')
@section('page-title', 'Daftar Cuti & Izin')
@section('page-subtitle', 'Review & Persetujuan Leave Request')

@section('content')
<div class="py-8">
  <div class="mb-6">
    <h1 class="text-xl font-bold text-gray-900">Daftar Cuti & Izin</h1>
  ...
  <div class="mb-6">
    ...
    <select onchange="window.location.href=this.value" 
      ...
      <option value="{{ route('user.leave-approvals.index') }}" {{ $status === 'all' ? 'selected' : '' }}>Semua Status</option>
      <option value="{{ route('user.leave-approvals.index', ['status' => 'pending']) }}" {{ $status === 'pending' ? 'selected' : '' }}>Pending</option>
      <option value="{{ route('user.leave-approvals.index', ['status' => 'approved']) }}" {{ $status === 'approved' ? 'selected' : '' }}>Approved</option>
      <option value="{{ route('user.leave-approvals.index', ['status' => 'rejected']) }}" {{ $status === 'rejected' ? 'selected' : '' }}>Rejected</option>
    </select>
  </div>
        ...
        <tbody class="divide-y divide-slate-100">
          @forelse($leaveList as $leave)
            <tr class="hover:bg-slate-50 transition-colors">
              ...
              <td class="px-4 py-3 text-center">
                <a href="{{ route('user.leave-approvals.show', $leave) }}" 
