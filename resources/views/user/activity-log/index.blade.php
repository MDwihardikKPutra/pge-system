...
<div class="py-8">
    <div class="bg-white border border-slate-200 rounded-lg shadow-sm">
        <div class="px-6 py-4 border-b border-slate-200 bg-primary-blue" style="background-color: #1e40af;">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <h2 class="text-base font-semibold text-white">Activity Log - Aktivitas Saya</h2>
                <form method="GET" action="{{ route('user.activity-log.index') }}" class="flex flex-wrap items-center gap-2">
                    ...
                    <button type="submit" class="px-3 py-1.5 text-xs font-medium bg-primary-blue text-white rounded hover:bg-primary-blue transition-colors" style="background-color: #1e40af;">Filter</button>
                    @if(request()->hasAny(['search', 'action', 'model_type', 'date_from', 'date_to']))
                        <a href="{{ route('user.activity-log.index') }}" class="px-3 py-1.5 text-xs font-medium border border-gray-300 text-gray-700 rounded hover:bg-gray-100 transition-colors">Reset</a>
                    @endif
                </form>
            </div>
        </div>
