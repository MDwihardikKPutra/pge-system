                    ...
                    <td class="px-4 py-3 text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            @if($purchase->status->value === 'approved')
                                @php
                                    $isAdmin = isset($isAdmin) ? $isAdmin : (auth()->user()->hasRole('admin'));
                                @endphp
                                <a href="{{ route($isAdmin ? 'admin.purchases.pdf' : 'user.purchases.pdf', $purchase) }}" target="_blank" class="inline-flex items-center gap-1 text-green-600 hover:text-green-800 text-xs font-medium transition-colors">
